...
def main():
    ...
    # Prepare model
    model = BertForPreTraining.from_pretrained(args.bert_model,config=BertConfig.from_pretrained(args.bert_model))
    model.to(device)
    ...
    if args.do_train:
        ...
        for epoch in trange(1, int(args.num_train_epochs) + 1, desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", position=0)):
                ...
                model.train()
                optimizer.zero_grad()
                prediction_scores, seq_relationship_score = model(input_ids=input_ids, attention_mask=input_mask,
