...
def convert_examples_to_features(item, train, bert_tokenizer):
    ...
    padding_length = 260 - len(input_ids)
...
class NeuralNetwork(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.bilinear = nn.Bilinear(200, 200, 1)
        '''
    def forward(self):
    ...
    def batch_att_cal(self, bertoutput, lenidx):
        ...
        for i in range(batchsize):
            context_evidence = hid_out[i, 1:lenidx[i][0]]
            response = hid_out[i, lenidx[i][0] + 1:lenidx[i][1]]
            ceattn = self.forward_attn(context_evidence, response)
            rattn = self.forward_attn(response, context_evidence)
            ceattn = self._classification(ceattn)
            rattn = self._classification(rattn)
            cls=self._classification(hid_out[i][0])

            output[i] = self.bilinear(ceattn, rattn) + self.linear(cls)
        return output.cuda()
    '''
    ...
    def fit(self, train, dev):  ############################여기가 메인임.
        ...
        self.optimizer = AdamW(optimizer_grouped_parameters, lr=self.args.learning_rate,
                               correct_bias=True)  # weight_decay=self.args.l2_reg, correct_bias=False)
